...
function AmenityIcon({ name }: { name: string }) {
  ...
  if (n.includes('breakfast') || n.includes('cafe'))
    ...
    <div className="space-y-2 w-full">
        ...
        return (
          ...
          >
            <div
              ...
              onClick={() =>
                setExpanded(isOpen ? null : h.result_id)
              }
            >
              {/* Stars badge */}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    {/* NEW: Distance from meeting venue */}
                    {h.distance_from_meeting != null && (
                      <div className="text-[10px] text-blue-600 dark:text-blue-400 font-medium mt-0.5">
                        📍 {h.distance_from_meeting.toFixed(1)} km from meeting
                        venue
                      </div>
                    )}
                  </div>
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    {h.is_mock && (
                      <span className="text-[10px] bg-amber-100 dark:bg-amber-900/30 text-amber-600 px-1 rounded">
                        sample
                      </span>
                    )}
                  </div>
              ...
              <div className="px-3 pb-3 pt-2 border-t border-gray-100 dark:border-gray-700 space-y-3">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {h.amenities.map((a) => (
                        <span
                          key={a}
                          className="flex items-center gap-1 text-xs bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-1 rounded-full"
                        >
                          <AmenityIcon name={a} />
                          {a}
                        </span>
                ...
                {h.review_count != null && (
                  <p className="text-xs text-gray-500">
                    {h.review_count.toLocaleString()} guest reviews
                  </p>
                )}
      ...
      {hotels.length > 3 && (
        <button
          ...
          className="w-full text-xs text-purple-600 dark:text-purple-400 hover:underline py-1 font-medium"
        >
          {showAll
            ? '↑ Show fewer'
